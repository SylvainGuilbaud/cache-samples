/*** Zen Module: ZMdemo_chui ***/

self._zenClassIdx['http://www.intersystems.com/zen/mojo/demo/chui/baseTemplate'] = 'ZMdemo_chui_baseTemplate';
self.ZMdemo_chui_baseTemplate = function(index,id) {
	if (index>=0) {ZMdemo_chui_baseTemplate__init(this,index,id);}
}

self.ZMdemo_chui_baseTemplate__init = function(o,index,id) {
	('undefined' == typeof _ZEN_Mojo_Component_contentTemplate__init) ?zenMaster._ZEN_Mojo_Component_contentTemplate__init(o,index,id):_ZEN_Mojo_Component_contentTemplate__init(o,index,id);
}
function ZMdemo_chui_baseTemplate_serialize(set,s)
{
	var o = this;s[0]='2350274433';s[1]=o.index;s[2]=o.id;s[3]=o.name;s[4]=set.addObject(o.parent,'parent');s[5]=set.addObject(o.composite,'composite');s[6]=o.align;s[7]=o.aux;s[8]=o.containerStyle;s[9]=(o.dragEnabled?1:0);s[10]=(o.dropEnabled?1:0);s[11]=(o.dynamic?1:0);s[12]=o.enclosingClass;s[13]=o.enclosingStyle;s[14]=o.error;s[15]=o.height;s[16]=(o.hidden?1:0);s[17]=o.hint;s[18]=o.hintClass;s[19]=o.hintStyle;s[20]=o.label;s[21]=o.labelClass;s[22]=o.labelDisabledClass;s[23]=o.labelStyle;s[24]=o.onafterdrag;s[25]=o.onbeforedrag;s[26]=o.ondrag;s[27]=o.ondrop;s[28]=o.onhide;s[29]=o.onrefresh;s[30]=o.onshow;s[31]=o.onupdate;s[32]=o.overlayMode;s[33]=o.renderFlag;s[34]=(o.showLabel?1:0);s[35]=o.slice;s[36]=o.title;s[37]=o.tuple;s[38]=o.valign;s[39]=(o.visible?1:0);s[40]=o.width;
}
function ZMdemo_chui_baseTemplate_getSettings(s)
{
	s['name'] = 'string';
	this.invokeSuper('getSettings',arguments);
}

self.ZMdemo_chui_baseTemplate_getData = function(key,criteria) {
var data = {};
switch (key) {
case 'home':
data.sectionHeader = 'Select a fruit';
break;
case 'details':
data.sectionHeader = criteria.product;
break;
}
return data;
}

self.ZMdemo_chui_baseTemplate_getLayout = function(key,criteria) {
var content = {};
switch(key) {
case 'home' :
content = {
children:[
{type:'$navbar',title:'=[sectionHeader]',children:[
]},
{type:'$article',children:[
{type:'$section',children:[
{type:'$list',caption:'Navigation',children:[
{type:'$listitem',showNavigation:true,key:'goto',value:'Apples',children:[
{type:'$h3',caption:'Apples'}
]},
{type:'$listitem',caption:'Oranges',showNavigation:true,key:'goto',value:'Oranges'},
{type:'$listitem',caption:'Bananas',showNavigation:true,key:'goto',value:'Bananas'},
]}
]}
]}
]
};
break;
case 'details':
content = {
children:[
{type:'$navbar',title:'=[sectionHeader]',children:[
{type:'$button',caption:'Back',key:'back'}
]},
{type:'$article',children:[
{type:'$section',children:[
]}
]}
]
};
break;
}
return content;
}

self.ZMdemo_chui_baseTemplate_onGetContent = function(providerName,key,criteria) {
switch (providerName)
{
case 'layout':
return this.getLayout(key,criteria);
case 'data':
return this.getData(key,criteria);
}
return null;
}

self.ZMdemo_chui_baseTemplate_onchange = function(key,value,final,docViewId) {
console.log('change ' + key + ' to ' + value + ' | ' + final);
var view = zen(docViewId);
switch(key) {
}
}

self.ZMdemo_chui_baseTemplate_onselect = function(key,value,docViewId) {
console.log('select '+key + ' | ' + value);
var mainView = zen(docViewId);
switch(key) {
case 'goto':
mainView.pushDocument('details',{product:value},'details',{product:value});
break;
case 'back':
mainView.popDocument();
break;
}
}

self.ZMdemo_chui_baseTemplate_ReallyRefreshContents = function() {
	zenInstanceMethod(this,'ReallyRefreshContents','','',arguments);
}
self.ZMdemo_chui_baseTemplate__Loader = function() {
	zenLoadClass('_ZEN_Mojo_Component_contentTemplate');
	ZMdemo_chui_baseTemplate.prototype = zenCreate('_ZEN_Mojo_Component_contentTemplate',-1);
	var p = ZMdemo_chui_baseTemplate.prototype;
	if (null==p) {return;}
	p.constructor = ZMdemo_chui_baseTemplate;
	p.superClass = ('undefined' == typeof _ZEN_Mojo_Component_contentTemplate) ? zenMaster._ZEN_Mojo_Component_contentTemplate.prototype:_ZEN_Mojo_Component_contentTemplate.prototype;
	p.__ZENcomponent = true;
	p._serverClass = 'ZMdemo.chui.baseTemplate';
	p._type = 'baseTemplate';
	p.serialize = ZMdemo_chui_baseTemplate_serialize;
	p.getSettings = ZMdemo_chui_baseTemplate_getSettings;
	p.ReallyRefreshContents = ZMdemo_chui_baseTemplate_ReallyRefreshContents;
	p.getData = ZMdemo_chui_baseTemplate_getData;
	p.getLayout = ZMdemo_chui_baseTemplate_getLayout;
	p.onGetContent = ZMdemo_chui_baseTemplate_onGetContent;
	p.onchange = ZMdemo_chui_baseTemplate_onchange;
	p.onselect = ZMdemo_chui_baseTemplate_onselect;
}
/* EOF */